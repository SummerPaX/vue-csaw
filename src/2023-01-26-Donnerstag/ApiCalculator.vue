...
import ApiCalculatorHistory from "./ApiCalculatorHistory.vue";
...
watch(expression, () => {
	...
	// set result to 0 and return from the watch function when expression is empty
	if (expression.value === "") {
		result.value = "0";
		return;
	}
...
<template>
	<div class="dark:bg-stone-800 dark:text-white flex flex-col items-center min-h-screen min-w-max">
		<div class="flex w-fit justify-between space-x-2 p-10">
			<div
				class="grid grid-cols-6 justify-items-center place-items-center gap-2 p-4 rounded-sm border-2 border-stone-900"
			>
				<p class="col-span-6 justify-self-end text-sm"><span>= </span>{{ result }}</p>
				<input
					v-model="expression"
					type="text"
				...
				<button
					@click="expression += '7'"
					class="bg-stone-200 hover:bg-stone-300 dark:bg-neutral-700 dark:hover:bg-neutral-600"
				>
					7
				</button>
				<button
				...
				<button
					@click="equalsPressed"
					class="bg-blue-500 text-white hover:bg-blue-400 dark:bg-blue-700 dark:hover:bg-blue-600"
				>
					=
				</button>
				<button @click="expression += '+'" class="">+</button>
			</div>
			<ApiCalculatorHistory @restoreHistory="(item) => (expression = item)" :history="history" />
		</div>
	</div>
</template>
