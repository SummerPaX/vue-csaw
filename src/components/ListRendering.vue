...
const filteredBeerList = computed(() => {
  return beerList.value.filter(beer =>
    beer.name.toLowerCase().includes(filter.value.toLowerCase())
  )
})
</script>
...
<template>
  <div class="bg-stone-800 text-white flex flex-col items-center min-h-screen min-w-max">
    <div class="flex flex-col w-fit">
      ...
      <template v-for="beer in filteredBeerList" :key="beer.id">
